...
convertIEA <- function(x, subtype) {
  ...
  if (subtype == "EnergyBalances-latest") {
    ...
    ieaShortNameMapping <- toolGetMapping("ieaShortNameMapping.csv", type = "sectoral", where = "mrcommonsenergy")
    ...
    flowMap <- dplyr::arrange(flowMap, .data$new_name)
    ...
    productMap <- dplyr::arrange(productMap, .data$new_name)
