...
convertEdgeBuildings <- function(x, subtype, subset) {
  ...
  if (subtype == "FE") {
    ...
    regionCol <- which(names(mapping) == "RegionCodeEUR_ETP")
    ...
    # until maxYearXinFE this will be exclusively FE,
    ...
    if (any(wfe < 0)) {
      warning(
        "FE weights computed by calcOutput('IOEdgeBuildings', subtype = X), ",
    ...
    weightSum <- toolAggregate(wfe, mappingfile, from = regionCol, to = isoCol, dim = 1)
    ...
    xadd <- toolAggregate(x, mappingfile,
      weight = wfe, from = regionCol, to = isoCol,
      zeroWeight = "allow"
    )
    ...
    namesToChange <- grep("(SSP1|SSP2|SDP|SDP_EI|SDP_RC|SDP_MC).*water_heating", getNames(result), value = TRUE)
    namesToChangeElec <- grep("elec", namesToChange, value = TRUE)

    regsOCD <- c("AUS", "CAN", "CHE", "NOR", "NZL")
    regTUR <- "TUR"
    endOfHistory <- 2015
    scenarioTime <- getYears(result, TRUE)[getYears(result, TRUE) > endOfHistory]

    whGrowth <- result[regsOCD, scenarioTime, namesToChange] -
      dimReduce(result[regsOCD, endOfHistory, namesToChange])
    whGrowth[, , namesToChangeElec] <- whGrowth[, , namesToChangeElec] * 0.5
    whGrowth[whGrowth < 0] <- 0
    whGrowthAgg <- dimSums(whGrowth, dim = 1)

    result[getItems(whGrowth, dim = "region"), getYears(whGrowth), getNames(whGrowth)] <-
      result[getItems(whGrowth, dim = "region"), getYears(whGrowth), getNames(whGrowth)] - whGrowth
    result[regTUR, getYears(whGrowth), getNames(whGrowth)] <-
      result[regTUR, getYears(whGrowth), getNames(whGrowth)] + whGrowthAgg
  }
  ...
  if (subtype == "Floorspace") {
    ...
    x <- time_interpolate(x, interpolated_year = remYearsHist, extrapolation_type = "constant")
    x <- toolAggregate(x[, remYearsHist, ], mappingfile,
      weight = wp,
      from = regionCol, to = isoCol
    )
    result <- x
  }
